...
namespace JK.Abp.RabbitMQ
{
    public class ChannelPool : IChannelPool, ISingletonDependency
    {
        ...
        public void Dispose()
        {
            ...
            if (!Channels.Any())
            {
                Logger.Debug($"Disposed channel pool with no channels in the pool.");
                return;
            }

            var poolDisposeStopwatch = Stopwatch.StartNew();

            Logger.Info($"Disposing channel pool ({Channels.Count} channels).");
            ...
            Logger.Info($"Disposed RabbitMQ Channel Pool ({Channels.Count} channels in {poolDisposeStopwatch.Elapsed.TotalMilliseconds:0.00} ms).");

            if(poolDisposeStopwatch.Elapsed.TotalSeconds > 5.0)
            {
                Logger.Warn($"Disposing RabbitMQ Channel Pool got time greather than expected: {poolDisposeStopwatch.Elapsed.TotalMilliseconds:0.00} ms.");
            }
